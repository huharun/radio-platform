...
export type ThemeConfig = { name: string; bg: string; surface: string; accent: string; accent2: string; orb1: string; orb2: string; text: string }
...
export default function ThemePanel({ current, onChange }: { current: ThemeConfig; onChange: (t: ThemeConfig) => void }) {
  ...
  return (
    <div className="theme-panel-wrap" ref={ref}>
      <button className="theme-toggle-btn" onClick={() => setOpen(o => !o)} title="Themes"><IconPalette /></button>
      {open && (
        <div className="theme-panel glass">
          <div className="theme-panel-title">Theme</div>
          <div className="preset-row">
            {PRESETS.map(p => (
              <button key={p.name} className={`preset-dot ${current.name === p.name ? "active" : ""}`}
                style={{ background: `linear-gradient(135deg, ${p.bg === "#f0f0f7" ? "#e0e0ef" : p.bg} 40%, ${p.accent})` }}
                onClick={() => onChange(p)} title={p.name} />
            ))}
          </div>
          <div className="custom-row">
            <label className="color-label"><span>BG</span><input type="color" value={custom.bg} onChange={e => setCustom(c => ({ ...c, bg: e.target.value }))} /></label>
            <label className="color-label"><span>Accent</span><input type="color" value={custom.accent} onChange={e => setCustom(c => ({ ...c, accent: e.target.value }))} /></label>
            <label className="color-label"><span>2nd</span><input type="color" value={custom.accent2} onChange={e => setCustom(c => ({ ...c, accent2: e.target.value }))} /></label>
          </div>
          <button className="apply-btn" onClick={() => onChange({ name: "custom", bg: custom.bg, surface: "rgba(255,255,255,0.07)", accent: custom.accent, accent2: custom.accent2, orb1: custom.accent + "55", orb2: custom.accent2 + "44", text: "rgba(255,255,255,0.95)" })}>Apply Custom</button>
        </div>
      )}
    </div>
  )
